...
import numpy as np
from models.PCT.misc.ops import FurthestPointSampler
from models.PCT.misc.ops import knn_point, index_points
...
class Point_Transformer(nn.Module):
    def __init__(self, feat_dim=128, output_channels=40):
        super(Point_Transformer, self).__init__()
        
        self.conv1 = nn.Conv1d(3, feat_dim, kernel_size=1, bias=False)
        ...
        self.sa1 = SA_Layer(feat_dim)
        self.sa2 = SA_Layer(feat_dim)
        self.sa3 = SA_Layer(feat_dim)
        self.sa4 = SA_Layer(feat_dim)

        self.conv_fuse = nn.Sequential(nn.Conv1d(feat_dim * 4, feat_dim * 8, kernel_size=1, bias=False),
                                   nn.BatchNorm1d(feat_dim * 8),
                                   nn.LeakyReLU(scale=0.2))

        self.linear1 = nn.Linear(feat_dim * 8, feat_dim * 4, bias=False)
        self.bn6 = nn.BatchNorm1d(feat_dim * 4)
        self.dp1 = nn.Dropout(p=0.5)
        self.linear2 = nn.Linear(feat_dim * 4, feat_dim * 2)
        self.bn7 = nn.BatchNorm1d(feat_dim * 2)
        self.dp2 = nn.Dropout(p=0.5)
        self.linear3 = nn.Linear(feat_dim * 2, output_channels)
