...
from models.PCT.networks.cls.pct import Point_Transformer, Point_Transformer2, Point_Transformer_Last, SA_Layer, Local_op, sample_and_group
...
class SimpleSkinModel(nn.Module):
    ...
    def __init__(self, feat_dim: int, pct_feat_dim: int, num_joints: int):
        super().__init__()
        self.num_joints = num_joints
        self.feat_dim = feat_dim

        self.pct = Point_Transformer(feat_dim=pct_feat_dim, output_channels=feat_dim)
...
def create_model(model_name='pct', feat_dim=256, **kwargs):
    if model_name == "pct":
        return SimpleSkinModel(feat_dim=feat_dim, num_joints=kwargs.get('num_joints', 52), pct_feat_dim=kwargs.get('pct_feat_dim', 128))
    if model_name == "pct2":
        return PCT2SkinModel(feat_dim=feat_dim, num_joints=kwargs.get('num_joints', 52))
